...
for obj in $OBJS; do
    ...
    if [ -f "$SRC_DIR/$dir/$base.ml" ]; then
        ...
        echo "Missing $base.ml"
    fi
    
    # Copy .mli file if it exists
    if [ -f "$SRC_DIR/$dir/$base.mli" ]; then
        cp "$SRC_DIR/$dir/$base.mli" "$DEST_DIR/src/"
    fi
    
    FLATTENED_OBJS="$FLATTENED_OBJS $base.cmx"
done
...
clean:
	rm -f src/*.cm[iox] src/*.o c2json
EOF

# Replace OBJ_LIST_HERE with the exact order but flattened to src/
SED_OBJS=""
for obj in $OBJS; do
    base=$(basename "$obj" .cmx)
    SED_OBJS="$SED_OBJS src/$base.cmx"
done
sed -i "s|OBJ_LIST_HERE|$SED_OBJS|g" "$DEST_DIR/Makefile"
